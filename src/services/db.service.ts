...
/**
 * Aggregate AI accuracy stats for the reporting endpoint.
 */
export const getAiAccuracyMetrics = async (dateFilter: Date | null) => {
  ...
  const [aggregate, categoryMatchCount, suggestionAcceptedCount, totalProcessed, recent] =
    await Promise.all([
      ...
      prisma.aiTicketMetric.count({ where: { ...where, final_category_id: { not: null } } }), // only evaluated rows
  ...
  return {
    ...
    evaluated_count:             totalProcessed,  // rows where admin has approved
